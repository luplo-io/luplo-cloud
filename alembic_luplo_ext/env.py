...
Driver choice
-------------
Migrations are raw SQL only (no async features needed), so this env
uses **sync psycopg3** instead of async asyncpg. The OSS DB URL in
production is libpq-style (`postgresql://...?sslmode=require`) reached
over a tailnet hostname; psycopg3-sync handles both like libpq does
(matching how `lp migrate` reaches the same DB), whereas asyncpg's
non-libpq DSN parsing + asyncio.create_connection path is fragile in
that environment.
"""
...
# Pin sync psycopg3 driver. Leave query string intact so libpq-style
# options (sslmode, etc.) reach psycopg3, which honours them.
_url = make_url(_raw_url)
if _url.drivername in ("postgresql", "postgres") or _url.drivername.startswith("postgresql+"):
