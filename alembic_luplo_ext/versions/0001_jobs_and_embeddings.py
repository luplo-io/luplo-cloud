...
existing OSS tables (glossary_terms, items) are written as raw SQL
...
def _require_pgvector() -> None:
    ...
    if row is None:
        raise RuntimeError(
            "pgvector extension is required for glossary-ext. Install it "
            "(e.g. CREATE EXTENSION vector) on the target Postgres before "
            "running glossary-ext alembic."
        )
